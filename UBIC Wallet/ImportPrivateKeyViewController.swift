...
class ImportPrivateKeyViewController: UIViewController {
    ...
    @IBAction func clickedImportPrivateKey(_ sender: Any) {
        ...
        if let newPrivateKey = privateKeyTextField.text {
            ...
            if keyStore.getUBICkeyStoreSize() >= 5 {
                let alert = UIAlertController(title: NSLocalizedString("Error", comment: ""), message: NSLocalizedString("Maximum number of private keys (5) has been reached", comment:""), preferredStyle: UIAlertController.Style.alert)
                alert.addAction(UIAlertAction(title: "OK", style: UIAlertAction.Style.default, handler: nil))
